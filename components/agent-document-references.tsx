...
import { Search, FileText, Plus, Trash2, Loader2, Save, Filter } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    documents.forEach(doc => {
      ...
      if (documentRefs[doc.category]) {
        ...
        const isSelected = 
          documentRefs[doc.category].includes(doc.id) || 
          documentRefs[doc.category].includes('*');
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Select documents that this agent can access for additional context
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <>
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center gap-2">
                <Checkbox 
                  id="select-all"
                  checked={filteredDocuments.every(doc => selectedDocuments[doc.id])}
                  ...
                  <div key={category} className="flex items-center justify-between p-3 border rounded-md">
                    <div className="flex items-center gap-2">
                      ...
                      <Label htmlFor={`category-${category}`} className="flex items-center">
                        <Badge variant="outline" className={`mr-2 ${getCategoryColor(category)}`}>
                          {category}
                        </Badge>
                        <span>{docsInCategory.length} document{docsInCategory.length !== 1 ? 's' : ''}</span>
                      </Label>
                    </div>
                    <Badge variant="outline">
                      {selectedInCategory}/{docsInCategory.length} selected
                    </Badge>
                  </div>
                );
              })}
